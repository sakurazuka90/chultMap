...
function init(){
	...
	for(var i = 0; i < 6120; i++){
		...
		var iterator = i;
		divnode.addEventListener('click', showCell);
		
		node.appendChild(divnode);
		list.appendChild(node);
	}
	
	const localStorageMapState = localStorage.getItem('mapState');
	console.log('State from local storage {}', localStorageMapState)
	
	if(localStorageMapState) {
	    makeCellsVisible(localStorageMapState);
	} else {
	...
	document.getElementById('hideToggle').addEventListener('change', hideToggle);
	document.getElementById('hideToggle').checked = hideMode;
	
	window.addEventListener('beforeunload', function (e) {
		    unmarkmarkCellsForHiding();
            localStorage.setItem('mapState', getVisibleCells());
    });
}
...
function getVisibleCells(){
	...
	return visibleCellsString;

}

function downloadStateFile() {
	var visibleCellsString = getVisibleCells();
...
function hideAllCells() {
	...
	for(var i = 1; i < liElements.length; i++) {
		liElements[i].firstElementChild.style.visibility = "visible";
		liElements[i].firstElementChild.addEventListener('click', showCell);
	}
}

function markCellsForHiding() {
	var list = document.getElementById("hexlist");
	var liElements = list.childNodes;
	
	for(var i = 1; i < liElements.length; i++) {
		if(liElements[i].firstElementChild.style.visibility === "hidden") {
			liElements[i].firstElementChild.style.visibility = "visible";
			//liElements[i].firstElementChild.style.backgroundColor = "yellow";
			liElements[i].firstElementChild.classList.remove("hex-grid__content");
			liElements[i].firstElementChild.classList.add("hex-grid__content_semi_opaque");
			liElements[i].firstElementChild.addEventListener('click', hideCell);
		}
	}
}

function unmarkmarkCellsForHiding() {
	var list = document.getElementById("hexlist");
	var liElements = list.childNodes;
	
	for(var i = 1; i < liElements.length; i++) {
		if(liElements[i].firstElementChild.classList.contains("hex-grid__content_semi_opaque")) {
			liElements[i].firstElementChild.classList.remove("hex-grid__content_semi_opaque");
			liElements[i].firstElementChild.classList.add("hex-grid__content");
			liElements[i].firstElementChild.addEventListener('click', showCell);
			liElements[i].firstElementChild.style.visibility = "hidden";
		}
	}
}

function showCell(e) {
	console.log("Show cell {}", e.target);
	
	if(hideMode) {
		e.target.classList.remove("hex-grid__content");
		e.target.classList.add("hex-grid__content_semi_opaque");
		e.target.removeEventListener('click', showCell);
		e.target.addEventListener('click', hideCell);
	} else {
...
function hideCell(e) {
	console.log("Hide cell {}", e.target);
	e.target.style.visibility = "visible";
	e.target.classList.remove("hex-grid__content_semi_opaque");
	e.target.classList.add("hex-grid__content");
	
	e.target.removeEventListener('click', hideCell);
	e.target.addEventListener('click', showCell);
}

function hideToggle(e) {
	console.log(e.target.checked);
	
	hideMode = e.target.checked;
